...
- `edge1`、`pvg1-edge` 与 `hutao-peer` 默认只暴露 lookglass proxy，由单独 frontend deployment 统一访问
...
def lookglass_config(node: Node) -> LookglassSpec | None:
    """为示例中的节点生成 proxy-only 的 lookglass 配置。"""

    if node.node_id not in {*NODES, EBGP_NODE.node_id}:
        return None
    return LookglassSpec(
        allowed_ips=[node.underlay_subnet],
        frontend_enabled=False,
        title_brand="Two internal + one eBGP lab",
...
def build_internal_state(node: Node) -> DesiredState:
    ...
    return DesiredState(
        ...
        dns=None,
        lookglass=lookglass_config(node),
...
def write_frontend() -> None:
    """写出独立 frontend 的 `docker run` 启动脚本。

    frontend 是一个加入三个节点 underlay 网络的独立 bird-lg-go 容器；
    项目已去 docker-compose 化，这里直接生成 Engine 原生命令脚本。
    """

    frontend_dir = OUTPUT_DIR / "frontend"
    frontend_dir.mkdir(parents=True, exist_ok=True)
    hkg1_net = f"dn42-{NODES['edge1'].node_id}_router_underlay"
    pvg1_net = f"dn42-{NODES['pvg1-edge'].node_id}_router_underlay"
    hutao_net = f"dn42-{EBGP_NODE.node_id}_router_underlay"
    (frontend_dir / "run-frontend.sh").write_text(
        dedent(
            f"""
            #!/bin/sh
            # 独立 bird-lg-go frontend：先创建容器并接入三个节点 underlay。
            set -eu

            docker rm -f dn42-lab-frontend 2>/dev/null || true
            docker create \\
              --name dn42-lab-frontend \\
              --restart unless-stopped \\
              --init \\
              -e 'BIRDLG_SERVERS=HKG1<10.254.51.2>,PVG1<10.254.52.2>,HUTAO<10.254.53.2>' \\
              -e 'BIRDLG_DOMAIN=' \\
              -e 'BIRDLG_LISTEN=:5000' \\
              -e 'BIRDLG_PROXY_PORT=8000' \\
              -e 'BIRDLG_TITLE_BRAND=Two internal + one eBGP lab' \\
              -e 'BIRDLG_NAVBAR_BRAND=DN42 Lab' \\
              -e 'BIRDLG_PROTOCOL_FILTER=Babel,BGP,OSPF' \\
              -e 'BIRDLG_WHOIS=/usr/bin/whois' \\
              -e 'BIRDLG_NET_SPECIFIC_MODE=dn42' \\
              -p 5000:5000 \\
              xddxdd/bird-lg-go:latest

            docker network connect '{hkg1_net}' dn42-lab-frontend
            docker network connect '{pvg1_net}' dn42-lab-frontend
            docker network connect '{hutao_net}' dn42-lab-frontend
            docker start dn42-lab-frontend
            """
        ).lstrip(),
        encoding="utf-8",
        newline="\n",
...
def write_readme() -> None:
    ...
    (OUTPUT_DIR / "README.md").write_text(
        dedent(
            ...
            This example renders four directories: `hkg1`, `pvg1`, `hutao`, and `frontend`.
            ...
            - `hkg1`, `pvg1`, and `hutao` expose only lookglass proxy sidecars inside their own underlay networks.
            - `frontend` contains a `docker run` script for a standalone bird-lg-go container that joins all three underlay networks.
