...
* bird-router 始终携带 ``/run/bird`` 控制 socket 的可写 volume——容器编排
...
def test_bird_router_carries_control_socket_mount() -> None:
    state = build_hkg1_example_state()
    bird = next(s for s in state.runtime.services if s.role == ServiceRole.BIRD_ROUTER)

    assert any(
        mount.source.endswith("runtime/bird-run")
        and mount.target == "/run/bird"
        and mount.readonly is False
        for mount in bird.volumes
    )
