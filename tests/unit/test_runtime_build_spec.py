...
* ports 接收旧式字符串（如 ``"127.0.0.1:5000:5000"``、``"32001:30001/udp"``）
  会被 Pydantic before-validator 归一化为 ``PortPublishSpec`` 对象，
  ``render_port_publish`` 可反转为多路径可读形式。
"""

import pytest

from dn42_runtime import render_router_dockerfile
from dn42_schemas import BuildSpec, PortPublishSpec, RouterDockerfileSpec, RuntimeServiceSpec, ServiceRole, render_port_publish
...
def test_runtime_service_ports_accept_legacy_strings_and_normalize() -> None:
    service = RuntimeServiceSpec(
        name="dn42-router-netns",
        role=ServiceRole.ROUTER_NETNS,
        # 显式喂入 legacy 字符串端口，验证 before-validator 归一化（字段声明
        # 是 list[PortPublishSpec]，故对类型检查器标注这是有意的宽松输入）。
        ports=["127.0.0.1:5000:5000", "32001:30001/udp"],  # pyright: ignore[reportArgumentType]
    )
