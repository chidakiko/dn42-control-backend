...
class NodeStatusStore:
    ...
    async def record_reresolve(self, report: WireGuardReresolveReport) -> None:
        ...
        纯信息性自愈观测：**不**进健康派生（不改 drift/health），只 append 到历史，
        供 status-events 展示「谁在什么时候因对端 IP 漂移被自动重设了 endpoint」。
        """

        payload = report.model_dump(mode="json")
        async with self._db.session() as session:
            # 确保节点行存在（节点已注册即有），但不触碰其健康字段。
            await self._get_or_create(session, report.node_id)
            session.add(
                NodeStatusEvent(
                    node_id=report.node_id,
                    kind="reresolve",
            ...
            await self._trim_history(session, report.node_id, "reresolve")
