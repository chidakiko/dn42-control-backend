...
async def _propagate_to_peers(
    ...
    """把 ``public_key`` 回填进所有"对端是 node_id"的 WG 接口，并重新物化对端节点。

    返回 {对端 node_id: 新世代号}。无内部对端时为空。
    """
    ...
    for row in rows.scalars():
        ...
        if not isinstance(peer, dict) or peer.get("public_key") == public_key:
            continue
        new_peer = dict(peer)
        new_peer["public_key"] = public_key
        spec["wireguard_peer"] = new_peer
        row.spec = spec  # 重新赋值以触发 JSON 列脏检测
