...
class PendingRegistrationStore:
    ...
    async def record(
        ...
        """落/刷新该节点的 pending 行（get-or-create）。

        并发下两个 register 可能都 SELECT-miss 再各插一条；DB 层的 partial unique index
        （一节点最多一条 pending）让败者撞 ``IntegrityError``，这里重试一次——此时 pending
        行已存在，走 update 分支幂等收敛。SQLite 单写者下走不到重试。
        """

        try:
            await self._record_once(requested_node_id, hostname=hostname, inventory=inventory)
