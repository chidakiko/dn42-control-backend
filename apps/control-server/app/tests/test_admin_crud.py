...
def _zone_payload(zone: str = "example.dn42") -> dict:
    return {
        "spec": {
            "zone": zone,
            "records_ref": f"zone://{zone}",
        }
    }
...
def test_provision_peering_without_bgp_only_creates_interface(client: TestClient, hkg1: str) -> None:
...
def test_failed_materialize_rolls_back_crud_change(
    ...
    try:
        row = conn.execute(
            "SELECT base_template FROM nodes WHERE node_id = ?", (hkg1,)
        ).fetchone()
...
def test_create_dns_zone(client: TestClient, hkg1: str) -> None:
    r = client.post(f"/api/v1/admin/nodes/{hkg1}/dns-zones", json=_zone_payload("lab.dn42"))
    assert r.status_code == 201, r.text

    snap = client.get(f"/api/v1/admin/nodes/{hkg1}/desired-state").json()
    zones = [z["zone"] for z in snap["dns"]["zones"]]
    assert "lab.dn42" in zones


def test_disabled_dns_zone_not_in_snapshot(client: TestClient, hkg1: str) -> None:
    r = client.post(f"/api/v1/admin/nodes/{hkg1}/dns-zones", json=_zone_payload("ephemeral.dn42"))
    zone_id = r.json()["id"]
    client.patch(f"/api/v1/admin/dns-zones/{zone_id}", json={"enabled": False})

    snap = client.get(f"/api/v1/admin/nodes/{hkg1}/desired-state").json()
    zones = [z["zone"] for z in snap["dns"]["zones"]]
    assert "ephemeral.dn42" not in zones
