...
def _post_routes(
    client: TestClient, node: str, token: str, routes: list[dict], captured: str
) -> None:
    payload = {
        "node_id": node,
...
def test_routing_ingest_and_query(client: TestClient, config: ControlServerConfig) -> None:
    ...
    # 前缀检索 + family 过滤
    v6 = client.get(f"/api/v1/admin/nodes/{node}/routing/prefixes", params={"family": "6"}).json()
    assert v6["total"] == 1
...
def test_routing_prefixes_replaced_on_new_snapshot(
    client: TestClient, config: ControlServerConfig
) -> None:
    """新快照整体替换明细表：被撤销的前缀不再出现在 get_prefixes。"""
    ...
    res = client.get(f"/api/v1/admin/nodes/{node}/routing/prefixes").json()
    assert res["total"] == 2
    assert {r["prefix"] for r in res["routes"]} == {"172.20.0.0/24", "172.21.0.0/24"}


def test_routing_prefixes_pagination(client: TestClient, config: ControlServerConfig) -> None:
    node = config.bootstrap_node_id
    token = config.bootstrap_agent_token
    routes = [_route(f"172.20.{i}.0/24", 64500) for i in range(5)]
    _post_routes(client, node, token, routes, "2025-01-01T00:00:00Z")

    page1 = client.get(
        f"/api/v1/admin/nodes/{node}/routing/prefixes", params={"limit": 2, "offset": 0}
    ).json()
    page2 = client.get(
        f"/api/v1/admin/nodes/{node}/routing/prefixes", params={"limit": 2, "offset": 2}
    ).json()
    assert page1["total"] == 5 and len(page1["routes"]) == 2
    assert len(page2["routes"]) == 2
    assert {r["prefix"] for r in page1["routes"]}.isdisjoint({r["prefix"] for r in page2["routes"]})


def test_routing_identical_repost_is_noop_churn(
    client: TestClient, config: ControlServerConfig
) -> None:
    """重复上报完全相同的全表（哈希一致 ⇒ 跳过明细重写）：明细仍可查、churn 归零。"""

    node = config.bootstrap_node_id
    token = config.bootstrap_agent_token
    routes = [_route("172.20.0.0/24", 64500), _route("fd42:1::/48", 64600, as_path=[64600])]
    _post_routes(client, node, token, routes, "2025-01-01T00:00:00Z")
    _post_routes(client, node, token, routes, "2025-01-01T00:05:00Z")

    res = client.get(f"/api/v1/admin/nodes/{node}/routing/prefixes").json()
    assert {r["prefix"] for r in res["routes"]} == {"172.20.0.0/24", "fd42:1::/48"}
    events = client.get(f"/api/v1/admin/nodes/{node}/routing/timeline").json()["events"]
    assert events[-1]["announced"] == 0 and events[-1]["withdrawn"] == 0
