...
from .bgp_sessions import SessionOut, _out as _bgp_out, _validate as _validate_bgp_spec
...
async def create_peering(
    ...
    async with db.session() as session:
        ...
        try:
            await session.flush()
        except Exception as exc:  # IntegrityError on UNIQUE(local_node_id, name)
            raise HTTPException(
...
async def _get_peering(session: Any, peering_id: int) -> Peering:
...
async def provision_peering(
    ...
    async with db.session() as session:
        ...
        try:
            await session.flush()
        except Exception as exc:  # UNIQUE(local_node_id, name)
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
        ...
        if bgp_spec is not None:
            bgp_row = BgpSession(
                ...
                peering_id=peering_row.id,
                name=bgp_spec.name,
                remote_asn=bgp_spec.remote_asn,
                enabled=bgp_spec.enabled,
                spec=bgp_spec.model_dump(mode="json"),
...
async def _existing_child_id(
    session: Any, model: type, node_id: str, name: str
) -> int | None:
...
async def _write_child(
    ...
    *,
    ...
    if row is None:
        row = model(node_id=node_id, name=name)
        session.add(row)
