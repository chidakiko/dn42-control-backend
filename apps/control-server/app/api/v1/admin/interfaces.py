...
def _iface_out(row: WgInterface) -> InterfaceOut:
    """``WgInterface`` 行 → API 响应（索引列读列，完整定义读 ``spec``）。"""
...
def _validate_iface_spec(spec: dict[str, Any]) -> InterfaceSpec:
    """校验 InterfaceSpec，失败抛 422（带字段级错误）。"""
...
async def _get_iface(session: AsyncSession, iface_id: int) -> WgInterface:
    """按 id 取接口，不存在抛 404。"""
...
async def update_interface(
    ...
    async with db.session() as session:
        ...
        if new_spec_validated is not None:
            row.apply_spec(new_spec_validated)
