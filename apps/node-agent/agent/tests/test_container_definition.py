...
def test_enabling_underlay_ipv6_triggers_recreate() -> None:
    """启用 underlay IPv6 → 直连容器 network.attach 含 ipv6_subnet → 哈希变 →
    声明式重建网络（带 IPv6），无需手动删环境。纯 IPv4 baseline 不含该键、不 churn。"""

    state = build_hkg1_example_state()
    data = state.model_dump(mode="json")
    data["runtime"]["underlay"]["ipv6_subnet"] = "fd00:dead:42::/64"
    data["runtime"]["underlay"]["ipv6_gateway"] = "fd00:dead:42::1"
    changed = state.__class__.model_validate(data)

    before = build_node_definitions(state, _DIR)["dn42-router-netns"]
    after = build_node_definitions(changed, _DIR)["dn42-router-netns"]

    assert "ipv6_subnet" not in before.payload["network"]["attach"]  # IPv4-only baseline 不churn
    assert before.config_hash != after.config_hash
    assert "network" in diff_payload_keys(before.payload, after.payload)
    assert after.payload["network"]["attach"]["ipv6_subnet"] == "fd00:dead:42::/64"
