...
def test_no_changes_yields_empty_plan() -> None:
    ...
    plan = build_convergence_plan(state, _container_plan(state, ContainerAction.KEEP), _noop_actions())
    assert plan.actions == []
...
def test_changed_apply_script_syncs_interface_without_wg_conf_change() -> None:
    ...
    state = build_hkg1_example_state()
    iface = next(
        i.name for i in state.interfaces if i.kind == InterfaceKind.WIREGUARD
    )
...
def test_wg_gateway_recreate_expands_full_replay_in_python() -> None:
    """netns/wg 重建 → 计划层在 Python 里展开为 loopback + 每个 WG 接口的独立动作
    （不再是一个容器内 bash glob 整体重放）。"""
    ...
    # 之后每个 WireGuard 接口各一个独立的 SYNC 动作（与渲染端枚举同源）。
    ...
    synced = [
        ...
        if a.kind == ConvergenceKind.WG_SYNC_INTERFACE
    ]
    assert synced == expected_ifaces
    assert expected_ifaces  # 样例确有 WG 接口，确保断言非空通过
    # 全量重放不应出现按接口拆除动作。
    assert not any(
        a.kind == ConvergenceKind.WG_REMOVE_INTERFACE for a in plan.actions
    )
...
def test_bird_recreate_skips_birdc_configure() -> None:
    ...
    plan = build_convergence_plan(
        state, _container_plan(state, ContainerAction.RECREATE), actions
    )
...
def test_execute_translates_actions_to_container_exec() -> None:
    ...
    plan = ConvergencePlan(
        actions=[
            ...
            ConvergenceAction(kind=ConvergenceKind.WG_SYNC_INTERFACE, container=wg, interface="wg-a"),
            ConvergenceAction(kind=ConvergenceKind.WG_REMOVE_INTERFACE, container=wg, interface="wg-b"),
        ]
    )
