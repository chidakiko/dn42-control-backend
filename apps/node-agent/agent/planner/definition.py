...
class ContainerDefinition:
    ...
    # payload 不可变(定义即冻结),哈希恒定。单次 reconcile 内 config_hash 被 planner /
    # labels / drift-reason / persist 访问 4~6 次,每次现算等于重复 canonical-JSON+SHA256。
    # 构造时一次算好缓存(frozen 数据类经 object.__setattr__ 写入 init=False 字段)。
    _config_hash: str = field(init=False, repr=False, compare=False, default="")

    def __post_init__(self) -> None:
        object.__setattr__(self, "_config_hash", payload_hash(self.payload))

    @property
    def config_hash(self) -> str:
        """payload 的 canonical JSON SHA-256 前 16 位:容器的内容寻址身份(构造时缓存)。"""
...
def _network_section(
    ...
    # underlay 的 IPAM（subnet/gateway/ipv6_*）必须进哈希：网络定义变化 → 网络要
    ...
    # 仅在启用 IPv6 时加键：纯 IPv4 节点哈希不变、不触发无谓重建；设/改/撤
    # ipv6_subnet ⇒ 直连容器哈希变 ⇒ 声明式重建网络（带 IPv6），无需手动删环境。
    if state.runtime.underlay.ipv6_subnet:
        attach["ipv6_subnet"] = state.runtime.underlay.ipv6_subnet
        attach["ipv6_gateway"] = state.runtime.underlay.ipv6_gateway
