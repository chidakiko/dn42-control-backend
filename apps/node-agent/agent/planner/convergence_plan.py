...
_APPLY_SUFFIX = ".sh"
_LOOPBACK_NAME = "dn42-lo"
_LOOPBACK_SCRIPT = "scripts/wg/apply-dn42-lo.sh"
# apply-dn42-lo.sh 走 loopback 专路；apply-all-wg.sh 是聚合入口，二者都不是单 WG 接口脚本。
_NON_INTERFACE_APPLY_NAMES = frozenset({"dn42-lo", "all-wg"})
# apply-all-wg.sh 是聚合入口，不对应任何具体接口（dn42-lo 仍是一个 dummy 接口，故不在此排除）。
_NON_NAMED_APPLY_SCRIPTS = frozenset({"all-wg"})
...
def build_convergence_plan(
    ...
    if netns_rebuilt or wg_rebuilt:
        # netns 内隧道全丢：在 Python 里展开"全量拉起" = loopback + 其余 dummy 接口
        # （dns-anycast 等）+ 每个 WG 接口，每个接口一个独立的可检查动作（取代容器内
        # bash glob 整体重放）。
        actions.append(
            ConvergenceAction(kind=ConvergenceKind.WG_SYNC_LOOPBACK, container=wg_container)
        )
        for iface in _managed_dummy_interface_names(state):
            actions.append(
                ConvergenceAction(
    ...
    # 其余 dummy 接口（dns-anycast 等）同样只在 apply-<name>.sh 里：脚本 create/update ->
    # 重跑该接口（ip link add dummy + ip addr replace）；脚本被删（DNS 退订）-> ip link del，
    # 否则任播地址会滞留在主机上继续吸引流量（黑洞）。
    dummy_changed, dummy_removed = _changed_dummy_interfaces(state, file_actions)
    for iface in dummy_changed:
        actions.append(
            ConvergenceAction(
                kind=ConvergenceKind.WG_SYNC_INTERFACE,
                container=wg_container,
...
def _wireguard_interface_names(state: DesiredState) -> list[str]:
    ...
    return sorted(iface.name for iface in state.interfaces if iface.kind == InterfaceKind.WIREGUARD)


def _managed_dummy_interface_names(state: DesiredState) -> list[str]:
    """枚举 dn42-lo 以外的 dummy 接口名（排序）。

    dn42-lo 走 loopback 专路（``WG_SYNC_LOOPBACK`` + 固定脚本名）；其余 dummy（如
    ``dns-anycast``，由 dns.bind_addresses 派生）和 WireGuard 一样每个一份
    ``apply-<name>.sh``，按接口名同步/拆除。
    """

    return sorted(
        iface.name
        for iface in state.interfaces
        if iface.kind == InterfaceKind.DUMMY and iface.name != _LOOPBACK_NAME
    )


def _changed_dummy_interfaces(
    state: DesiredState, actions: list[PlanAction]
) -> tuple[list[str], list[str]]:
    """从 file plan 提取需要同步 / 拆除的非 loopback dummy 接口名。

    dummy 接口没有 wg conf，地址只在 ``scripts/wg/apply-<name>.sh`` 里：
    - create/update 且仍是当前 dummy -> 同步（重建接口 + 同步地址）。
    - delete 且已不在当前任何接口集、且没有对应 ``wireguard/<name>.conf`` 删除（与 WG 拆除
      区分，避免重复计数）-> 拆除该 dummy（``ip link del``）。
    """

    managed = set(_managed_dummy_interface_names(state))
    current = managed | {iface.name for iface in state.interfaces} | {_LOOPBACK_NAME}
    conf_deleted = {
        conf
        for action in actions
        if action.action == "delete" and (conf := _wg_conf_interface(action.path)) is not None
    }
    ...
    for action in actions:
        if action.action == "noop":
            continue
        name = _apply_script_name(action.path)
        if name is None:
            continue
        if action.action in ("create", "update"):
            if name in managed:
                changed.add(name)
        elif action.action == "delete" and name not in current and name not in conf_deleted:
            removed.append(name)
...
def _bird_files_changed(actions: list[PlanAction]) -> bool:
    return any(
        action.path.startswith(_BIRD_PREFIX) and action.action != "noop" for action in actions
    )
...
def _changed_wg_interfaces(
    ...
    wg_names = {iface.name for iface in state.interfaces if iface.kind == InterfaceKind.WIREGUARD}
    changed: set[str] = set()
    removed: list[str] = []
    for action in actions:
        if action.action == "noop":
...
def _apply_script_interface(path: str) -> str | None:
    """`scripts/wg/apply-<iface>.sh` -> WireGuard 接口名；loopback / 聚合脚本 / 其余返回 None。"""
...
def _apply_script_name(path: str) -> str | None:
    """`scripts/wg/apply-<name>.sh` -> 接口名（含 dn42-lo 等 dummy）；聚合脚本 / 其余返回 None。"""

    if not path.startswith(_APPLY_PREFIX) or not path.endswith(_APPLY_SUFFIX):
        return None
    name = path[len(_APPLY_PREFIX) : -len(_APPLY_SUFFIX)]
    if not name or "/" in name or name in _NON_NAMED_APPLY_SCRIPTS:
        return None
    return name
