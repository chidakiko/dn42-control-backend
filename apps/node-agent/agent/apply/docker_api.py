...
def plan_underlay_network(
    ...
    *,
    ...
    network = existing[0]
    network.reload()
    if _network_matches(network, subnet=subnet, gateway=gateway):
    ...
    *,
    ...
    client.networks.create(
        ...
        ipam=docker_types.IPAMConfig(
            pool_configs=[docker_types.IPAMPool(subnet=subnet, gateway=gateway)]
        ),
...
def _network_matches(network: Any, *, subnet: str, gateway: str) -> bool:
    config = network.attrs.get("IPAM", {}).get("Config", [])
    for candidate in config:
        candidate_subnet = candidate.get("Subnet")
        if not candidate_subnet or ":" in candidate_subnet:
            continue
        return candidate_subnet == subnet and candidate.get("Gateway") == gateway
    return False
