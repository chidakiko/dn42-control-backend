...
def render_desired_state(state: DesiredState) -> list[RenderedFile]:
    ...
    for interface in sorted(state.interfaces, key=lambda item: item.name):
        # 每个 dummy 接口（dn42-lo 身份地址、dns-anycast 任播服务地址……）渲染一份
        # apply-<name>.sh：幂等创建 dummy + ip addr replace 同步地址。
        if interface.kind == InterfaceKind.DUMMY:
            files.append(
                RenderedFile(
                    f"scripts/wg/apply-{interface.name}.sh",
                    render_dummy_apply_script(interface),
                )
            )
        elif interface.kind == InterfaceKind.WIREGUARD:
            files.append(
