...
# CoreDNS（dns 角色）框架托管服务的规范参数。节点分配了有效 DNS 组（=启用 DNS）时，
# DesiredState 校验按需注入该服务（见 desired_state._normalize_dns_runtime）；渲染产物
# `coredns/Corefile` + `coredns/zones/*` 经 ``DNS_CONFIG_SOURCE`` 挂载暴露给 CoreDNS。
DNS_SERVICE_NAME = "dn42-dns"
DNS_SERVICE_IMAGE = "coredns/coredns:1.12.1"
DNS_CONFIG_SOURCE = "coredns"
DNS_CONFIG_TARGET = "/etc/coredns"

# 框架托管的 DNS 任播接口名。节点订阅 DNS 组（=启用 DNS）时，DesiredState 校验据
# ``dns.bind_addresses`` 合成这条 dummy 接口（承载任播服务地址），并把它登记为
# ``track_service`` dummy ⇒ BIRD direct_anycast 起源对应 /32、/128（见
# desired_state._normalize_dns_anycast）。bind_addresses 是 DNS 服务地址的唯一真源：
# 未订阅的节点既无该接口也不宣告，任播地址只出现在真正提供 DNS 的节点上。
DNS_ANYCAST_INTERFACE = "dns-anycast"
...
class PortPublishSpec(StrictModel):
    ...
    def validate_ranges(self) -> "PortPublishSpec":
        ...
        if (
            self.host_port is not None
            and self.host_port_end is None
            and self.container_port_end is not None
        ):
            object.__setattr__(
                self,
                "host_port_end",
        ...
        if (
            self.container_port_end is not None
            and self.host_port_end is not None
...
class UnderlayNetworkSpec(StrictModel):
    ...
    Attributes:
        name: underlay 网络名。
        subnet: underlay IPv4 子网 CIDR。
        gateway: underlay IPv4 网关地址。
        ipv6_subnet: 可选的 underlay IPv6 ULA 子网 CIDR。设置后 agent 建网络时启用 IPv6
            （`enable_ipv6` + 该 IPAM pool），容器获得 IPv6 + 默认路由，宿主经 NAT66 转出公网
            IPv6——让容器能拨 IPv6-only 对端 endpoint。为 `None` 时网络保持纯 IPv4（现状）。
        ipv6_gateway: underlay IPv6 网关地址；设置 `ipv6_subnet` 时必填。
    """
    ...
    @classmethod
    def validate_ipv6_gateway(cls, value: str | None) -> str | None:
        if value is not None:
            ip_address(value)
        return value

    @model_validator(mode="after")
    def validate_ipv6_pair(self) -> "UnderlayNetworkSpec":
        if self.ipv6_subnet and not self.ipv6_gateway:
            raise ValueError("ipv6_gateway is required when ipv6_subnet is set")
        return self
...
class RouterRuntimeSpec(StrictModel):
    ...
    def validate_project_name(cls, value: str | None) -> str | None:
        ...
        if not re.fullmatch(r"[a-z0-9][a-z0-9_-]*", normalized):
            raise ValueError(
                "project_name must contain only lowercase letters, digits, underscores, and hyphens"
            )
        return normalized
    ...
    def validate_services(self) -> "RouterRuntimeSpec":
        ...
        missing_network_services = sorted(
            target
            for target in (
                _network_mode_service(service.network_mode) for service in enabled_services
            )
...
def _validate_role_requirements(service: RuntimeServiceSpec) -> None:
    ...
    if service.role in {ServiceRole.WG_GATEWAY, ServiceRole.BIRD_ROUTER} and not service.command:
        raise ValueError(
            f"runtime service {service.name} role {service.role.value} requires command"
        )
...
def render_port_publish(port: PortPublishSpec) -> str:
    ...
    return (
        f"{prefix}{container}/{port.protocol}" if port.protocol != "tcp" else f"{prefix}{container}"
    )
