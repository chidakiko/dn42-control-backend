...
DNS / WireGuard 接口）的示例；`examples/rendered-hkg1/` 均由
本函数输出经完整管线渲染而成，是
...
def build_hkg1_example_state() -> DesiredState:
    ...
    return DesiredState(
            ...
            services=[
                ...
                RuntimeServiceSpec(
                    ...
                    volumes=[
                        ...
                        VolumeMount(source="scripts", target="/opt/dn42/scripts"),
                        # 控制 socket 运行目录：宿主 agent 直连 bird.ctl 采集路由。
        ...
        interfaces=[
            InterfaceSpec(
                # dn42-lo 只承载节点身份地址；DNS 任播服务地址（172.20.0.20/.22 等）不再写死在
                # 这里，而是由 dns.bind_addresses 单源派生到框架托管的 dns-anycast 接口
                # （见 desired_state._normalize_dns_anycast）。
                name="dn42-lo",
...
_LAB_LINKS = [
    ...
    _LabLink(
        ...
        "198.18.10.2",
        "198.18.10.3",
        "fdce:1111:2222:ff11::0",
        "fdce:1111:2222:ff11::1",
        15,
    ),
    _LabLink(
        "edge2",
        ...
        "198.18.10.4",
        "198.18.10.5",
        "fdce:1111:2222:ff12::0",
        "fdce:1111:2222:ff12::1",
        20,
    ),
]
...
_LAB_EBGP_LINKS = {
    ...
    "edge3": _LabEbgpLink(
        ...
        "198.18.20.4",
        "198.18.20.5",
        "fdce:1111:2222:ef2b::0",
        "fdce:1111:2222:ef2b::1",
    ),
}
...
def _lab_published_wireguard_ports(node: _LabNode) -> list[PortPublishSpec]:
    ...
    for link in _lab_ebgp_links_for_node(node.node_id):
        peer_node_id = (
            _LAB_EBGP_NODE.node_id
            if node.node_id != _LAB_EBGP_NODE.node_id
            else link.internal_node_id
        )
        ports.append(
            PortPublishSpec(
...
def _lab_build_state(node: _LabNode) -> DesiredState:
    ...
    return DesiredState(
        ...
        runtime=RouterRuntimeSpec(
            underlay=UnderlayNetworkSpec(
                subnet=node.underlay_subnet, gateway=node.underlay_gateway
            ),
