...
并按对「单一真相源」重构最关键的维度给每个地址分类。本文只讲地址的**概念**（类型 / 用途 /
真相源 / 影响半径），不含具体部署的地址数值。
...
| link-local `fe80::/64` | OSPFv3 稳定邻接身份；潜在的 LLA eBGP 传输源 | 现在在接口 `addresses` 里（**节点级 `link_local` 字段尚不存在于 schema/DB**，只在 demo 脚本的 `Node` 里有） | 源 | 一条链路（若收敛成节点级 LLA 则变节点级） |

> 寻址风格说明：现状是 **per-link 子网**（每条链路一对 `/31`+`/127`，本端地址逐链路不同）。
> 若改成「节点单 `/32`+`/128`+一个 LLA、所有链路复用」，本层地址才可能上移到第 1 层（节点级）。
> 两种风格的取舍与迁移代价属后续重构提案，不在本文。

### 第 4 层：任播服务地址 —— 影响半径：跨节点共享 + 注册表

| 地址 | 是什么 / 用途 | 真相源 | 分类 | 影响半径 |
|---|---|---|---|---|
| `dns.bind_addresses` | DNS 任播服务地址。**唯一真源**：normalize 派生 `dns-anycast` dummy 接口（v4→`/32`、v6→`/128`）+ 登记 `track_service` ⇒ BIRD 起源任播前缀进 BGP | `DnsGroup.bind_addresses` | 源 | 订阅同组的**所有节点**（anycast）；注册表登记的任播段 |
| `dns-anycast` 接口地址 + 任播前缀 | 上者的派生产物 | —（派生自 `bind_addresses`） | **派生（黄金范例）** | 同上 |
| DNS 记录值（A/AAAA/PTR 的 content） | 解析到 loopback / 任播地址 | `DnsRecord.content` | 副本（指向 loopback 的手工绑定） | 解析正确性；renumber 节点后必须跟改 |

`dns.bind_addresses → dns-anycast` 这条链是**全项目最干净的单一真相源实现**，重构请直接对标它。

### 第 5 层：runtime / underlay（容器层）—— 影响半径：节点内部

仅在单节点容器编排内有意义，不进公网身份/任播分区。
...
4. **DNS A/AAAA/PTR 记录** ← 指向 loopback 的手工绑定；renumber 节点必须连带改。
