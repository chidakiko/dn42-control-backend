...
全表，解析出每条路由的 `prefix` / `origin_asn` / `as_path` / `next_hop` / 来源 protocol /
是否最优 / 是否本地起源，并按 BIRD 的 ROA 表（`dn42_roa` / `dn42_roa_v6`）做 RFC 6811
起源校验（三态 `valid` / `invalid` / `not-found`；无法判定即 ROA 整表没采到时留空、不计入
任何桶），全量上报 `POST /agent/routing-table` 供控制面做 Radar 式分析。

**采集走直连 BIRD 控制 socket**（`bird.ctl`），不再经 `docker exec` 跑 `birdc` 子进程：
agent 在宿主上自实现 BIRD 远程控制协议（读 greeting、发命令、按状态码分帧剥码，输出
与 `birdc` 一致），对 socket 依次发 `show route table master4/master6 all`、ROA 表、以及每个
eBGP 对端的 `show route import table`（过滤前 RPKI 分布）。直连省掉 Docker Engine API
`exec_run` + 容器内 fork `birdc` 两层壳，并可流式读取上万条路由，不必把整张表整块 buffer。

- **socket 路径**默认从 bird-router 服务的 `/run/bird` **可写挂载**推导（宿主侧
  `<rendered_dir>/runtime/bird-run/bird.ctl`）。该挂载是 bird-router 的**一等不变量**——由
  `DesiredState` 校验（`dn42_schemas` 的 `_normalize_bird_control_socket`）**无条件注入**，
  不随任何 sidecar 启停变化；bird daemon 默认把控制 socket 写在 `/run/bird/bird.ctl`，经此
  挂载暴露到宿主供 agent 直连。
- 可用 `bird_socket_path`（TOML / `DN42_AGENT_BIRD_SOCKET_PATH`）**显式覆盖**路径；非常规
  部署 / 联调才需要。bird 服务缺 `/run/bird` 挂载（快照待升级）时本轮采集标 `unavailable`。
