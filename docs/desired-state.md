...
`DesiredState` 是 Control Server 和 Node Agent 之间的主协议。它描述一个节点应该具备的网络身份、runtime 服务、接口、BGP session、DNS 配置和模板版本。
...
1. `Node.base_template` 提供 `runtime`、`bird`、`templates` 和 `dns` 顶层骨架。
