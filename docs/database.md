...
| `apps/control-server/app/db/models/routing.py` | `NodeRouting`、`NodeRouteEntry`、`NodeRoutingEvent` |
...
erDiagram
    ...
    nodes ||--|| node_routing : route_table
    nodes ||--o{ node_route_entries : route_detail
    nodes ||--o{ node_routing_events : route_churn
...
| `node_routing.node_id -> nodes.node_id` | `ON DELETE CASCADE` |
| `node_route_entries.node_id -> nodes.node_id` | `ON DELETE CASCADE` |
| `node_routing_events.node_id -> nodes.node_id` | `ON DELETE CASCADE` |
...
| `labels` | `JSON` | 自定义标签 |
| `base_template` | `JSON` | `runtime`、`bird`、`templates`、`dns` 等顶层骨架 |
...
每个节点一行，保存 Agent 上报的 BIRD 路由全表的**预聚合**与表规模计数（与 reconcile 健康分开
——路由全表体量更大、节奏更慢）。逐路由明细不再存这里，见 `node_route_entries`。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `node_id` | `String(64)`，主键 | 所属节点（FK `nodes`，CASCADE） |
| `observation` | `String(16)` | `observed` / `unavailable` / `not-observed`；非 observed 不清空已有全表 |
| `captured_at` | `DateTime(timezone=True)` 或 null | 采集时刻 |
| `route_count` / `_v4` / `_v6` | `Integer` | 去重后唯一前缀数（≠ 总路径数） |
| `rpki_valid` / `_invalid` / `_not_found` | `Integer` | RPKI 三态计数 |
| `aggregates` | `JSON` | origins / prefix_lengths / as_path_lengths / peers / prefilter / `routes_hash` 等预聚合 |
| `routes` | `JSON` 或 null | **历史遗留列，恒写 `None`**；明细已迁到 `node_route_entries`，覆盖旧 blob 回收空间 |
| `updated_at` | `DateTime(timezone=True)` | 行更新时刻 |

### node_route_entries

逐路由明细，**每路由一行**——取代早期把整张表（上万条）塞进 `node_routing.routes` 单个 JSON 列、
每周期全量覆盖写的做法。前缀检索（`GET /admin/nodes/{id}/routing/prefixes`）走 SQL `WHERE` +
索引 + `LIMIT/OFFSET`，按 family / scope / 关键字过滤全部下推 DB，不再把数 MB JSON 读进内存全扫。

`record_snapshot` 在全表内容变化时整表重写该节点明细（delete + bulk insert），并按
`aggregates.routes_hash` **门控**：内容未变则跳过，避免稳定期反复重写上万行。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | `Integer` | 自增主键 |
| `node_id` | `String(64)` | 所属节点（FK `nodes`，CASCADE） |
| `prefix` | `String(64)` | CIDR 前缀 |
| `is_v6` / `local` / `primary` | `Boolean` | 地址族 / 本地起源 / 是否最优（前两者建索引供过滤） |
| `origin_asn` | `Integer` 或 null | 起源 AS |
| `protocol` | `String(128)` 或 null | 来源 BIRD protocol |
| `rpki` | `String(16)` 或 null | `valid` / `invalid` / `not-found` / null |
| `next_hop` | `String(128)` 或 null | 下一跳 |
| `as_path` / `communities` / `large_communities` | `JSON` 或 null | 路径与 community 列表 |

索引：`(node_id, is_v6)`、`(node_id, local)`、`(node_id, prefix)`（均以 `node_id` 打头）。

### node_routing_events

append-only 时间序列，每次全表快照落一条计数 + 相对上一次的 churn（announced / withdrawn 前缀数），
供 Radar 式趋势图。每节点最多保留最近 500 条。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | `Integer` | 自增主键 |
| `node_id` | `String(64)`，索引 | 所属节点（FK `nodes`，CASCADE） |
| `captured_at` | `DateTime(timezone=True)` 或 null | 采集时刻 |
| `route_count` / `_v4` / `_v6` | `Integer` | 表规模计数 |
| `rpki_valid` / `_invalid` / `_not_found` | `Integer` | RPKI 三态计数 |
| `announced` / `withdrawn` | `Integer` | 相对上一快照的前缀增减 |
| `created_at` | `DateTime(timezone=True)`，索引 | 落库时刻 |
...
| `e5f6a7b8c9d0_wireguard_key_escrow.py` | 节点级 WireGuard 公钥 + 私钥托管 |
| `f6a7b8c9d0e1_remove_compose_runtime_fields.py` | 去 docker-compose 化：剥离存量 `DesiredState` JSON 里的 compose 字段 |
| `a7b8c9d0e1f2_slim_build_spec.py` | 剥离 `build.context` / `build.dockerfile` |
| `b8c9d0e1f2a3_node_lifecycle.py` | `nodes` 增加 `lifecycle`（退役流程） |
| `c9d0e1f2a3b4_node_route_entries.py` | 新增 `node_route_entries` 路由明细索引表（明细从 `node_routing.routes` JSON 列迁出） |
