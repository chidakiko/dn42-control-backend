...
| `routing_interval_seconds` | number | `300.0` | `DN42_AGENT_ROUTING_INTERVAL_SECONDS` | — | 路由全表旁路采集间隔（秒）；设 `0` 关闭。仅常驻 + 接入控制面时启用，见 [node-agent.md](node-agent.md#路由全表观测routingtablesnapshot) |
| `bird_socket_path` | string 或 null | null | `DN42_AGENT_BIRD_SOCKET_PATH` | — | BIRD 控制 socket 路径的**显式覆盖**；留空时从 bird-router 的 `/run/bird` 挂载自动推导。路由采集直连该 socket（不再经 docker exec 跑 birdc） |
